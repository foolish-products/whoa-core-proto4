...
int glowiter = 0;
int next = 0;
int activeChannelCount = 4 ;
void onOffGlowIter() {
  next = (glowiter + 1) % (activeChannelCount + 1);  
  glow[next] = glow[glowiter];
  glow[glowiter] = !glow[glowiter]; 
  glowiter = next;
}
...
void flowGlowIter() {
  ...
  glowiter = (glowiter + 1) % (activeChannelCount);
  glow[glowiter] = true;
  
//  next = (glowiter + 1) % (activeChannelCount);  
//  
//  glow[next] = !glow[next];
//  glow[glowiter] = !glow[glowiter]; 
//  glowiter = next;
...
unsigned long glowIterTime = millis();
unsigned long iterDelay = 500;
float scale = 1.0;
float currScale = 1.0;
float pscale[] = {1.0, 1.0, 1.0, 1.0};



int measureIter = 0;
float currMeasure = 0;
float minMeasure = 10000; // big number
float maxMeasure = 0;
...
//  delay(1000); 
 ...
 //   delay(7);
    measureIter = (measureIter + 4) % activeChannelCount;
    val = sense(measureIter);
    Serial.println(val);
    Serial.print(measureIter);
    Serial.print(" ");
    Serial.print(currMeasure - minMeasure);
    Serial.print(" ");
    Serial.println((val / 1024.0) * 3.3 / 3.0);

    

//    prevVal[0][loopIter % activeChannelCount] = val;
//
//    currMeasure = 0; 
//
//    for (int i = 0; i < activeChannelCount; i++) {
//      currMeasure += prevVal[0][i];
//    }
//
//    Serial.println((currMeasure / 1024.0) * 3.3 / 3.0);
//    
//    if (minMeasure > currMeasure) { 
//      minMeasure = currMeasure;
//      
//    }
//    else if (maxMeasure < currMeasure) { 
//      maxMeasure = currMeasure;
//    }
//
//    currScale = (currMeasure - minMeasure) / (maxMeasure - minMeasure) *  (currMeasure - minMeasure) / (maxMeasure - minMeasure) *  (currMeasure - minMeasure) / (maxMeasure - minMeasure);
//  //  *  (currMeasure - minMeasure) / (maxMeasure - minMeasure);
//    scale = 0.0;
//    for (int i = 0; i < 2; i++) { 
//      pscale[i] = pscale[i + 1];
//      scale += pscale[i];
...
//    }
    ...
    if (millis() - glowIterTime > long(blah)) {
      flowGlowIter();
      switchOutputs(glow);
      delay(blah);
      flowGlowIter();
...
//      if ((millis() - modeTime) < 120000) { 
//        loopIter ++; 
//        
//        flowGlowIter();
//      }
//      else if ((millis() - modeTime) < 240000) { 
//        loopIter ++;
//        
//        onOffGlowIter();
//      }
...
//         prevVal[(iter - 2) % prevValSize][0] - val > 70) { 
